// TAB — Eksploatacja. Funkcjonalna kopia Problemów (drag-reorder, galeria, chat, archiwizacja),
// osobna kolekcja danych. Logika współdzielona z Problemami/Zakupami → useCardTab (patrz cardTab.js).
...
export default {
  ...
  setup() {
    return useCardTab("eksploatacja", { newTitle: "Nowy wątek" });
  },
  template: `
  <div class="cardtab">
    <header class="cardtab__bar">
      ...
      >{{ p.label }}</button>
      <button class="prio-add" @click="addCardMenu($event)" title="Dodaj wątek"><i class="ph-fill ph-plus"></i></button>
    ...
    <div class="cardtab__strip" ref="stripEl" @wheel="onWheel">
      ...
      >
        ...
        <div class="karta__footer">
          <button class="btn-final" @click="archiveCard(c)">Zakończ wątek</button>
        </div>
      </article>

      <!-- Placeholder: nowy wątek — tylko gdy w kolekcji nie ma jeszcze żadnej karty (jest już mały "+" przy filtrach) -->
      <div v-if="isEmpty" class="karta karta--add" @click="addCard()">
        <span class="karta-add__plus"><i class="ph-fill ph-plus"></i></span>
        <span class="karta-add__lbl">Dodaj<br>Wątek</span>
      </div>
