// TAB 3 — Zakupy. Karty zamówień: drag-reorder, chat, archiwizacja.
// Logika współdzielona z Problemami → useCardTab (patrz cardTab.js).
...
export default {
  ...
  setup() {
    return useCardTab("zakupy", { newTitle: "Nowy zakup" });
  },
  template: `
  <div class="cardtab">
    <header class="cardtab__bar">
      ...
      >{{ p.label }}</button>
      <button class="prio-add" @click="addCardMenu($event)" title="Dodaj zakup"><i class="ph-fill ph-plus"></i></button>
    ...
    <div class="cardtab__strip" ref="stripEl" @wheel="onWheel">
      ...
      >
        ...
        <div class="karta__body">
          ...
          <!-- Media — dodawanie przez menu flagi ("Dodaj media"), nie przez stały
               przycisk. Układ: 1 zdjęcie = duże; 2 = obok siebie; 3+ = duże + reszta małe. -->
          ...
          <div class="krow">
            ...
            <div class="krow__cnt">
              <textarea
                class="karta__opis karta__opis--tall"
                v-autogrow="c.opis"
                v-spellfocus
                v-livemodel="c.opis"
        ...
        <div class="karta__footer">
          <button class="btn-final" @click="archiveCard(c)">Zamówienie dostarczone</button>
        </div>
      </article>

      <!-- Placeholder: nowy zakup — tylko gdy w kolekcji nie ma jeszcze żadnej karty (jest już mały "+" przy filtrach) -->
      <div v-if="isEmpty" class="karta karta--add" @click="addCard()">
        <span class="karta-add__plus"><i class="ph-fill ph-plus"></i></span>
        <span class="karta-add__lbl">Dodaj<br>Zakup</span>
      </div>
